  ...
  const topicData = {
    'math-algebra-ed': {
      ...
      resources: [
        ...
        { id: 'r2', title: 'Practice Problems Set 1', url: 'https://example.com/algebra-practice-1.pdf', description: '50 practice problems with solutions' }
      ],
      quizzes: [
        {
          id: 'quiz1',
      ...
      <motion.div variants={itemVariants} className="mb-12">
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className={`bg-gradient-to-r ${getSubjectColor(topic.subject)} p-8 text-white`}>
            ...
            <p className="text-lg opacity-90 mb-6">{topic.description}</p>
            {progress > 0 && (
          ...
          <div className="space-y-8">
            {topic.videos.length > 0 ? (
              ...
              <div className="bg-white rounded-xl shadow-lg p-8 text-center">
                ...
                <p className="text-gray-600 mb-6">Videos for this topic haven't been added yet. Help us grow by contributing content!</p>
          ...
          <div>
            {!selectedQuizId ? (
              <div className="grid gap-6 md:grid-cols-2 mb-8">
                {quizzes.length === 0 ? (
                  <div className="bg-white rounded-xl shadow-lg p-8 text-center col-span-2">
                    <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                      <Trophy className="w-8 h-8 text-gray-400" />
                    </div>
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">No Quizzes Available</h3>
                    <p className="text-gray-600 mb-6">No quizzes have been added for this topic yet. Help us grow by contributing content!</p>
                    <Link to="/contribute" className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-500 to-teal-500 text-white rounded-lg hover:from-blue-600 hover:to-teal-600 transition-all duration-200 shadow-md hover:shadow-lg">
                      Contribute Quizzes
                    </Link>
                  </div>
                  ...
                  quizzes.map((quiz) => (
                    <button
                      key={quiz.id}
                      className="bg-white rounded-xl shadow-lg p-8 text-center hover:scale-[1.03] transition-transform border border-gray-200"
                      onClick={() => setSelectedQuizId(quiz.id)}
                    >
                      <Trophy className="w-8 h-8 text-blue-500 mx-auto mb-4" />
                      <h3 className="text-xl font-bold text-gray-900 mb-2">{quiz.title}</h3>
                      <span className="text-gray-600 text-sm">{quiz.questions.length} questions</span>
                    </button>
                  ))
                )}
              </div>
              ...
              <div>
                <button className="mb-6 text-blue-600 hover:underline text-lg font-medium" onClick={() => setSelectedQuizId(null)}>
                  ← Back to Quiz List
                </button>
                {(() => {
                  const quiz = quizzes.find((q) => q.id === selectedQuizId);
                  if (!quiz) return null;
                  return <Quiz questions={quiz.questions} title={quiz.title} />;
                })()}
              </div>
            )}
          </div>
      ...
      {(topic.videos.length === 0 && topic.resources.length === 0 && quizzes.length === 0) && (
        <motion.div variants={itemVariants} className="mt-12">
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center">
            ...
            <p className="text-yellow-700 mb-6">This topic is being prepared. Check back soon for videos, resources, and quizzes!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/contribute" className="inline-flex items-center px-6 py-3 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors">
                <Plus className="w-4 h-4 mr-2" />
                Contribute Content
              </Link>
              <Link to="/contact" className="inline-flex items-center px-6 py-3 border border-yellow-600 text-yellow-600 rounded-lg hover:bg-yellow-50 transition-colors">
                Request Content
              </Link>
            </div>
          </div>
